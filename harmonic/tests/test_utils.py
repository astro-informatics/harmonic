...
def test_split_data():
    ...
    chains_train, chains_test = ut.split_data(chains_all, training_proportion=training_proportion)
    ...
    with pytest.raises(ValueError):
        chains_train, chains_test = ut.split_data(chains_all, training_proportion=-0.1)

    with pytest.raises(ValueError):
        chains_train, chains_test = ut.split_data(chains_all, training_proportion=1.5)

    with pytest.raises(ValueError):
        chains_train, chains_test = ut.split_data(chains_all, training_proportion=1E-10)
...
def test_validation_fit_indexes():
    ...
    with pytest.raises(ValueError):
        ut.validation_fit_indexes(ncross, nchains_in_val_set, ncross, indexes)
    with pytest.raises(ValueError):
        ut.validation_fit_indexes(-1, nchains_in_val_set, ncross, indexes)

    indexes_val, indexes_fit = ut.validation_fit_indexes(0, nchains_in_val_set, ncross, indexes)
    ...
    indexes_val, indexes_fit = ut.validation_fit_indexes(1, nchains_in_val_set, ncross, indexes)
    assert len(indexes_val) == 3
    ...
    indexes_val, indexes_fit = ut.validation_fit_indexes(2, nchains_in_val_set, ncross, indexes)
...
def test_cross_validation():
    ...
    with pytest.raises(ValueError):
        ut.cross_validation(chains, [], hyper_parameters_KDE, MODEL="not_a_model")


    # just checks the result of the code is unchanged
    validation_variences = ut.cross_validation(chains, [np.array([1E-1,1E1])], \
    ...
    validation_variences = ut.cross_validation(chains, [], hyper_parameters_KDE, \
                        verbose=False)
