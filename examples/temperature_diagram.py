...
def get_moon(sigma, resolution=1024):
    ...
    components_distribution=tfd.MultivariateNormalDiag(loc=coords.T, scale_diag=jnp.full(coords.shape[0], sigma))
    )
    return distribution
...
model.fit(posterior_samples, jnp.zeros(batch_size), epochs=epochs_num)
...
samples_train = MCSamples(samples=posterior_samples,names = names, labels = labels, label = 'Posterior')
...
g.plot_2d([ samples_train, samples2, samples3], 'x0', 'x1', colors = ['orange', 'green', '#800080'], filled=True)
