...
def run_example(
    ...
    for burn_iteration in range(burn_iterations):
        hm.logs.info_log(
            "Run burn sampling for burning subiteration {}...".format(
                burn_iteration + 1
            )
        )
        # Clear memory
        ...
        (pos, prob, rstate) = sampler.run_mcmc(
            pos, nburn / burn_iterations, rstate0=rstate
        )
    ...
    for chain_iteration in range(chain_iterations):
        hm.logs.info_log(
            "Run sampling for chain subiteration {}...".format(chain_iteration + 1)
        )
        ...
        sampler = emcee.EnsembleSampler(nchains, ndim, ln_posterior, args=[inv_cov])
        (pos, prob, rstate) = sampler.run_mcmc(
            pos, (samples_per_chain - nburn) / chain_iterations, rstate0=rstate
        )
    ...
    # ======================================================================
    hm.logs.info_log(
        "Ln Inv Evidence: analytic = {}, estimate = {}".format(
            ln_rho, cal_ev.ln_evidence_inv
        )
    )
    hm.logs.info_log(
        "Ln Inv Evidence: 100 * |analytic - estimate| / |analytic| = {}%".format(
            100.0 * np.abs((cal_ev.ln_evidence_inv - ln_rho) / ln_rho)
        )
    )
    # ======================================================================
    ...
    hm.logs.debug_log("---------------------------------")
    hm.logs.debug_log(
        "lnargmax = {}, lnargmin = {}".format(cal_ev.lnargmax, cal_ev.lnargmin)
    )
    hm.logs.debug_log(
        "lnprobmax = {}, lnprobmin = {}".format(cal_ev.lnprobmax, cal_ev.lnprobmin)
    )
    hm.logs.debug_log(
        "lnpredictmax = {}, lnpredictmin = {}".format(
            cal_ev.lnpredictmax, cal_ev.lnpredictmin
        )
    )
    hm.logs.debug_log("---------------------------------")
    hm.logs.debug_log(
        "shift = {}, shift setting = {}".format(cal_ev.shift_value, cal_ev.shift)
    )
    hm.logs.debug_log("running sum total = {}".format(sum(cal_ev.running_sum)))
...
if __name__ == "__main__":
    ...
    hm.logs.debug_log(
        "Number of additional chain iterations = {}".format(chain_iterations)
    )
