...
def run_example(
    ...
    thin = True
    thin_by = 10**3 #10**3 #these should be a factor of samples per chain
    training_thin_by = 5
    load_model = False
    ...
    if not thin:
        thin_by = 1
    skip_batches = 20
    samples_used_size = int(samples_per_chain/thin_by * ((chain_iterations-skip_batches-1)/chain_iterations)) #store samples used for inference
    print(samples_used_size)
    samples_used = np.empty((nchains, samples_used_size, ndim))
    if estimate_evidence:
        hm.logs.info_log("Compute evidence...")
        temps_to_consider = [0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8]
        ln_evidence_inv_summary = np.zeros((len(temps_to_consider), 6))
        
        ix_start = 0
        for i_T in range(len(temps_to_consider)):
            temp = temps_to_consider[i_T]

            print(temp)

            i = chain_iterations - 1 
            with open(
                    "examples/plots/samples_rosenbrock_"
                    + str(ndim)
                    + "D_"
                    ...
                    samples = np.load(f)
                    samples = samples[:, ix_start::training_thin_by, :]
                    print("Training samples shape after thinning", samples.shape)
            ...
            with open(
                    ...
                    lnprob = np.load(f)
                    lnprob = lnprob[:, ix_start::training_thin_by]
            ...
            if load_model:
                ...
                load_lab = "examples/models" + save_name_start[14:] + "_start" + str(ix_start) + "_model"
                if flow_type == "RealNVP":
                    model = hm.model.RealNVPModel.deserialize(load_lab)
                if flow_type == "RQSpline":
                    model = hm.model.RQSplineModel.deserialize(load_lab)
            else:
                ...
                model_lab = "examples/models/" + save_name_start[14:] + "_start" + str(ix_start) + "_model"
                model.serialize(model_lab)

            model.temperature = temp

            ev = hm.Evidence(nchains, model)

            for i in tqdm(range(skip_batches, chain_iterations-1)):
                ...
                with open(
                    ...
                    if thin:
                        samples = samples[:, ix_start::thin_by, :]
                        if i == skip_batches:
                            print("samples shape after thinning", samples.shape)
                ...
                with open(
                    ...
                    lnprob = np.load(f)
                if thin:
                    lnprob = lnprob[:, ix_start::thin_by]
                ...
                """
                #print("start ", (i)*int(samples_per_chain/(thin_by*chain_iterations)), " end ", (i+1)*int(samples_per_chain/(thin_by*chain_iterations)))
                samples_used[:, (i-skip_batches)*int(samples_per_chain/(thin_by*chain_iterations)):(i+1-skip_batches)*int(samples_per_chain/(thin_by*chain_iterations)), :] = samples
                ev.add_chains(chains)

                if i == chain_iterations -2:
                    ln_evidence, ln_evidence_std = ev.compute_ln_evidence()
                    err_ln_inv_evidence = ev.compute_ln_inv_evidence_errors()
                    print("Temperature is ", temp, " start ix is ", ix_start)
                    print(
                        "ln evidence = {} +/- {} {}".format(
                            -ev.ln_evidence_inv,
                    ...
                    ln_evidence_inv_summary[i_T, 0] = ev.ln_evidence_inv
                    ln_evidence_inv_summary[i_T, 1] = err_ln_inv_evidence[0]
                    ln_evidence_inv_summary[i_T, 2] = err_ln_inv_evidence[1]
                    ln_evidence_inv_summary[i_T, 3] = ev.ln_evidence_inv_var
                    ln_evidence_inv_summary[i_T, 4] = ev.ln_evidence_inv_var_var
                    ln_evidence_inv_summary[i_T, 5] = temp
            ...
            if plot_corner:
                ...
                samples_inference = samples_used.reshape((-1, ndim))

                if False:
                    hm.utils.plot_getdist(samples_inference)
                    if savefigs:
                ...
                hm.utils.plot_getdist_compare(
                    samples_inference, samps_compressed, legend_fontsize=12.5
                )
                if savefigs:
                    save_name = (
                        save_name_start
    ...
    save_name = (
        save_name_start
        + "_rosenbrock_ln_evidence_inv_vary_T.dat"
    )
    np.savetxt(
        save_name,
